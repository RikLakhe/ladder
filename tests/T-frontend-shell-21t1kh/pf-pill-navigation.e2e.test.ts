...
describe("B-3: clicking a PF pill navigates to that PF's page with the level tab strip", () => {
  it("competency page links to /primary-functions/:pfId", async () => {
    const res = await fetch(`${BASE_URL}/competencies/${competencyId}`);
    const body = await res.text();
    expect(body).toContain(`/primary-functions/${pfId}`);
  });

  it("PF page returns 200 and renders a level tab strip", async () => {
